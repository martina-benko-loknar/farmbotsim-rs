...
def main():
    ...
    summary = summarize(
        df, group_cols=["field_size"], value_cols=VALUE_COLS
    ).sort_values("field_size")

    print(summary)

    os.makedirs(EXPORTS_DIR, exist_ok=True)
    summary_path = f"{EXPORTS_DIR}/field_comparison_summary.csv"
    summary.to_csv(summary_path, index=False)
    print(f"Saved summary to {summary_path}")

    os.makedirs(FIGURES_DIR, exist_ok=True)
    group_labels = summary["field_size"].astype(str).tolist()
    ...
    generate_grouped_bar_comparison_plot(
        ...
        xlabel="field size (/)", ylabel="evaluations",
        output_dir=FIGURES_DIR, prefix="field_comparison_evaluations",
    )
    ...
    generate_grouped_bar_comparison_plot(
        ...
        xlabel="field size (/)", ylabel="$E_{\\mathrm{tot}}$ (Wh)",
        output_dir=FIGURES_DIR, prefix="field_comparison_energy",
    )
    ...
    generate_grouped_bar_comparison_plot(
        ...
        xlabel="field size (/)", ylabel="time (s)",
        output_dir=FIGURES_DIR, prefix="field_comparison_time",
    )
